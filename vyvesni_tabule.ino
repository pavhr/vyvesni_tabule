...
void setup() {
  ...
  for(int i= 0; i<analogRead(Pot2);i++){
    analogWrite(Out2, i);
    delay(rychlost);
  }
  for(int i= 0; i<analogRead(Pot3);i++){
    analogWrite(Out3, i);
    delay(rychlost);
  }
}
...
void loop() {
  while(1){ // cas na rozsveceni svetel
    analogWrite(Out1, analogRead(Pot1));
    analogWrite(Out2, analogRead(Pot2));
    analogWrite(Out3, analogRead(Pot3));
  }
